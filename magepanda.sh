...
TIMES="10"
CHATID="6749565987"
KEY="6860114753:AAH_KNokHf0NK7zrEPuZR7Pua5lwrPoq0so"
...
# // Banner
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "  Welcome To AtaLioMego™ Project ${YELLOW}(${NC}${green} Stable Edition ${NC}${YELLOW})${NC}"
echo -e " This Will Quick Setup VPN Server On Your Server"
echo -e "  Author : ${green}╭───────────AtaLioMego™────────────╮${NC}${YELLOW}(${NC} ${green} 88Numb Tunneling ${NC}${YELLOW})${NC}"
echo -e " © Recode by 88Numb & Nirandroid Team${YELLOW}(${NC} 2023 ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 2
###### IZIN SC 
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/esthadn1603/ItsNotYou/main/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/esthadn1603/ItsNotYou/main/ip | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://raw.githubusercontent.com/esthadn1603/ItsNotYou/main/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# REPO    
    REPO="https://raw.githubusercontent.com/ataliomegk/VIP/main/"
...
print_install "Membuat direktori xray"
    ...
    mkdir -p /var/lib/88numb >/dev/null 2>&1
    # // Ram Information
...
function base_package() {
    clear
    ########
    print_install "Packages Installing"
    ...
    print_success "Finish Installing"
...
echo "IP=" >> /var/lib/88numb/ipvps.conf
...
restart_system() {
    USRSC=$(wget -qO- https://raw.githubusercontent.com/esthadn1603/ItsNotYou/main/ip | grep $ipsaya | awk '{print $2}')
    EXPSC=$(wget -qO- https://raw.githubusercontent.com/esthadn1603/ItsNotYou/main/ip | grep $ipsaya | awk '{print $3}')
...
<code>────────────────────</code>
<i>Automatic Notification from Github AtaLioMego</i>
"'&reply_markup={"inline_keyboard":[[{"text":"ᴏʀᴅᴇʀ🐳","url":"https://t.me/ataliomego"},{"text":"ɪɴꜱᴛᴀʟʟ🐬","url":"https://t.me/ataliomego"}]]}'
...
rm -rf /etc/vmess/.vmess.db
    ...
    mkdir -p /etc/88numb/limit/vmess/ip
    mkdir -p /etc/88numb/limit/vless/ip
    mkdir -p /etc/88numb/limit/trojan/ip
    mkdir -p /etc/88numb/limit/ssh/ip
...
wget raw.githubusercontent.com/Aataliomego/VIP/main/limit/limit.sh && chmod +x limit.sh && ./limit.sh
...
clear
# banner
echo "Banner /etc/88numb.txt" >>/etc/ssh/sshd_config
sed -i 's@DROPBEAR_BANNER=""@DROPBEAR_BANNER="/etc/klmpk.txt"@g' /etc/default/dropbear

# Ganti Banner
wget -O /etc/88numb.txt "${REPO}limit/issue.net"
